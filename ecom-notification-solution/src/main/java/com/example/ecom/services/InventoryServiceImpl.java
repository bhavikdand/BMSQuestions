...
public class InventoryServiceImpl implements InventoryService{
    ...
    public Inventory updateInventory(int productId, int quantity) throws ProductNotFoundException {
        ...
        for (Notification notification : notifications) {
            String emailBody = String.format("Dear %s, \n\n The product %s is now available in stock. \n\n Regards, \n Team Ecom", notification.getUser().getName(), notification.getProduct().getName());
            emailAdapter.sendEmail(notification.getUser().getEmail(), "Product in stock", emailBody);
